...
def pic(filename,divide_n):
    ...
    min_x = min(data)
    max_x = max(data)
    x = np.linspace(-10,10, divide_n)  # 找到随机点的最小，最大值，形成分布区间
    ...
    standard = np.exp(-np.power(x,2)/2)/np.sqrt(2*np.pi)  # 标准p函数
...
pic("poisson_2.txt",100)
pic("poisson_5.txt",100)
pic("poisson_10.txt",100)

pic("exp_2.txt",100)
pic("exp_5.txt",100)
pic("exp_10.txt",100)

pic("uniform_2.txt",100)
pic("uniform_5.txt",100)
pic("uniform_10.txt",100)

pic("gauss_2.txt",100)
pic("gauss_5.txt",100)
pic("gauss_10.txt",100)